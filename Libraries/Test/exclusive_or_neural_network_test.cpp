...
protected:
  ...
  const NeuralNetworkArchitecture neural_network_architecture_{
      2, 3, 1, ActivationFunctionType::SIGMOID};
...
TEST_F(
  ...
  EXPECT_NEAR(expected_result[0], hidden_layer_.at(0).activation_func_result,
              1e-4);
  // Second hidden neuron and it's children in the output layer
  ApplyActivationFunctionOnNeuronsValue(synapses_.find(3));
  EXPECT_NEAR(expected_result[1], hidden_layer_.at(1).activation_func_result,
              1e-4);
  // Third hidden neuron and it's children in the output layer
  ApplyActivationFunctionOnNeuronsValue(synapses_.find(4));
  EXPECT_NEAR(expected_result[2], hidden_layer_.at(2).activation_func_result,
              1e-4);

  ApplyActivationFunctionOnNeuronsValue(synapses_.find(5));
  EXPECT_NEAR(expected_result[3], output_layer_.at(0).activation_func_result,
              1e-4);
}
